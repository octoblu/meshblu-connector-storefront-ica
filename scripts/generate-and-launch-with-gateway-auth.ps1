...
Version: 0.9
.DESCRIPTION
A Powershell v3 Script that utilizes invoke-webrequest to create, download and launch an desktop via Citrix ICA file from Storefront.  Script uses gateway authentication.
.PARAMETER sfurl
...
.EXAMPLE
.\generate-and-launch-with-gateway-auth.ps1 -sfurl "https://go.citrite.net/Citrix/StoreWeb/" -icapath "D:\myica.ica" -domain "citrix.com" -desktop "Managed Win10 LAS Desktop"
#>
Param
(
...
Invoke-WebRequest -Uri ($sfurl + "GatewayAuth/Login") -Method POST -Headers $headers -WebSession $SFSession|Out-Null
...
$username = Invoke-WebRequest -Uri ($sfurl + "Authentication/GetUserName") -Method POST -Headers $headers -WebSession $SFSession
Write-Host "Authed Username $username"
