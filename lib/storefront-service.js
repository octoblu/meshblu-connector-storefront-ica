...
const debug = require("debug")("meshblu-connector-storefront-ica:StoreFrontService")
...
class StoreFrontService {
  ...
  explicitLoginAttempt(callback) {
    ...
    request.post(options, (error, response) => {
      if (error) return callback(error)
      debug('explicitLoginAttempt result', response.statusCode, response.body)
      if (response.statusCode > 399) {
  ...
  generateICA(callback) {
    ...
    series({
      ...
      if (error) return callback(error)
      debug('generateICA result', result)
